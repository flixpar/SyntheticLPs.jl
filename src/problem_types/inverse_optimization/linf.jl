...
"""
function InverseLPMaxErrorProblem(
...
function build_model(prob::InverseLPMaxErrorProblem)
    ...
    for j in 1:n
        @constraint(model, prob.deviation_weights[j] * (c[j] - prob.prior_cost[j]) <= max_dev)
        @constraint(model, prob.deviation_weights[j] * (prob.prior_cost[j] - c[j]) <= max_dev)
    end
