...
function _inverse_network_dimensions(target_variables::Int, average_degree::Float64)
    ...
    preferred_k = target < 150 ? 2 : target < 800 ? 4 : 6
    for nodes in 4:max_nodes
        edges = clamp(round(Int, average_degree * nodes / 2),
                      nodes - 1, nodes * (nodes - 1) ÷ 2)
        for observations in max(2, preferred_k - 2):(preferred_k + 2)
            count = 6 * edges + observations * nodes
            candidate = (error=abs(count - target), nodes=nodes, edges=edges,
...
function _inverse_grid_coordinates(
    ...
    for node in 1:n_nodes
        ...
        coordinates[node, 1] = spacing * column +
            rand(rng, Uniform(-jitter, jitter)) * spacing
        coordinates[node, 2] = spacing * row +
            rand(rng, Uniform(-jitter, jitter)) * spacing
    end
    return coordinates
end

function _inverse_undirected_edges(
    rng::AbstractRNG,
    ...
    for u in 1:n_nodes
        for offset in (width, width - 1, width + 1, 2)
            ...
            distance = hypot(coordinates[u, 1] - coordinates[v, 1],
                             coordinates[u, 2] - coordinates[v, 2])
            push!(candidates, (distance * rand(rng, Uniform(0.90, 1.10)),
                               pair[1], pair[2]))
        end
    end
    sort!(candidates)
...
function _inverse_road_data(
    ...
    for (u, v) in undirected_edges
        distance = max(0.15, hypot(coordinates[u, 1] - coordinates[v, 1],
                                  coordinates[u, 2] - coordinates[v, 2]))
        ...
        elseif profile == :regional_roads
            ...
            draw = rand(rng)
            draw < 0.45 ? :local : draw < 0.78 ? :arterial : :highway
        end
        speed_range = road_class == :local ? (24.0, 42.0) :
                      road_class == :arterial ? (42.0, 68.0) :
                      road_class == :regional ? (58.0, 85.0) : (88.0, 118.0)
    ...
    while !isempty(heap)
        ...
        if !isempty(heap)
            ...
            while true
                ...
                right = left + 1
                child = right <= length(heap) && heap[right] < heap[left] ?
                        right : left
        ...
        while cursor <= length(queue)
            ...
            for e in adjacency[u]
                ...
                ((arc.tail == removed.tail && arc.head == removed.head) ||
                 (arc.tail == removed.head && arc.head == removed.tail)) && continue
...
function _inverse_path_observations(
    ...
    n_nodes = size(coordinates, 1)
    diameter = hypot(maximum(coordinates[:, 1]) - minimum(coordinates[:, 1]),
                ...
                for candidate_source in 1:n_nodes
                    for candidate_destination in 1:n_nodes
                        ...
                        !allow_reuse &&
                            ...
                            continue
                        candidate_distances, predecessor =
                            _inverse_shortest_distances(
                                n_nodes, arcs, costs, candidate_source,
                            )
                        candidate_path = _inverse_reconstruct_path(
                            arcs, predecessor,
...
function _inverse_paths_are_optimal(
    ...
    return all(observations) do observation
        distances, _ = _inverse_shortest_distances(
            n_nodes, arcs, costs, observation.source,
        )
...
function _inverse_make_prior_informative!(
    ...
    for observation in observations, edge in observation.path_arcs
        ...
        other_path_cost = sum(
            (prior_cost[e] for e in observation.path_arcs if e != edge);
            init=0.0,
        )
        prior_cost[edge] = max(
            prior_cost[edge],
            alternative - other_path_cost + 0.05 * alternative,
        )
        _inverse_paths_are_optimal(
            n_nodes, arcs, observations, prior_cost,
        ) || return true
    end
    return false
end
...
function InverseShortestPathProblem(
    ...
    n_nodes, n_edges, K = _inverse_network_dimensions(
        target_variables, profile_data.degree,
    )
    coordinates = _inverse_grid_coordinates(
        rng, n_nodes, profile_data.side_km, profile_data.jitter,
    )
    undirected_edges = _inverse_undirected_edges(rng, coordinates, n_edges)
    arcs, true_cost = _inverse_road_data(
        rng, profile_data.name, coordinates, undirected_edges,
    )
    observations, potentials = _inverse_path_observations(
        rng, coordinates, arcs, true_cost, K,
    )
    ...
    for _ in 1:100
        prior_cost = true_cost .*
            rand(rng, LogNormal(0.0, 0.24), length(true_cost))
    ...
    if _inverse_paths_are_optimal(n_nodes, arcs, observations, prior_cost)
        _inverse_make_prior_informative!(
            n_nodes, arcs, observations, prior_cost,
...
function build_model(prob::InverseShortestPathProblem)
    ...
    @objective(
        model,
        Min,
        sum(prob.deviation_weight[e] *
            (deviation_positive[e] + deviation_negative[e]) for e in 1:E),
    )
    @constraint(
        model,
        prior_deviation[e in 1:E],
        arc_cost[e] - prob.prior_cost[e] ==
            deviation_positive[e] - deviation_negative[e],
    )
    @constraint(
        model,
    ...
    @constraint(
        model,
        potential_anchor[k in 1:K],
        potential[k, prob.observations[k].source] == 0.0,
    )
    @constraint(
        model,
