...
function LayeredInverseShortestPathProblem(
    ...
    arc_budget = target_variables > num_nodes ?
                 max(length(tail), (target_variables - num_nodes) ÷ 3) : length(tail)
    ...
    for e in 1:num_arcs
        ...
        true_cost[e] = e in path_set ? reduced :
                       max(reduced, 0.0) + rand(rng, Uniform(2.0, 12.0))
    end
    ...
    for _ in 1:100
        ...
        _layered_prior_is_optimal(num_nodes, source, sink, tail, head,
                                  prior_cost, path_arcs) || break
    end
    if _layered_prior_is_optimal(num_nodes, source, sink, tail, head,
                                 prior_cost, path_arcs)
    ...
    witness = feasibility_status == feasible ?
        LayeredPathWitness(copy(potentials), copy(true_cost)) : nothing
    certificate = nothing
    ...
    return LayeredInverseShortestPathProblem(
        num_nodes, num_arcs, layers, width, tail, head, layer_of, source, sink,
        path_arcs, prior_cost, cost_lower, cost_upper, deviation_weights,
...
function build_model(prob::LayeredInverseShortestPathProblem)
    ...
    for e in 1:E
        @constraint(model,
                    potential[prob.head[e]] - potential[prob.tail[e]] <= arc_cost[e])
        if e in path_set
            @constraint(model,
                        potential[prob.head[e]] - potential[prob.tail[e]] == arc_cost[e])
        end
...
function _layered_shortcut_certificate_is_valid(prob::LayeredInverseShortestPathProblem)
    ...
    return certificate.shortcut_arc ∉ prob.path_arcs &&
           isapprox(certificate.shortcut_upper,
                    prob.cost_upper[certificate.shortcut_arc]) &&
           isapprox(certificate.bypassed_lower_sum,
