...
function ReservesEnergyProblem(
    ...
    spin_fraction = rand(rng, Uniform(0.05, 0.15))
    nonspin_fraction = spin_fraction * rand(rng, Uniform(0.5, 1.0))
    ...
    for name in sources
        ...
        non_spinning_reserve_cost[name] = gc * rand(rng, Uniform(0.02, 0.08))
    end
