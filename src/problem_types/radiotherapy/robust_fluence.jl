...
function RobustFluenceIMRTProblem(
    ...
    case, spec, conflict_indices, rng = _rt_build_case(
        target_variables, :robust_fluence, feasibility_status, seed; n_scenarios=n_scenarios
    )
    ...
    for scenario in 2:n_scenarios
        ...
        push!(shifts, (radius * cos(angle), radius * sin(angle), 0.20 * (2rand(rng) - 1)))
    end
    matrices = SparseMatrixCSC{Float64, Int}[case.dose_matrix]
    for shift in shifts[2:end]
        matrix = _rt_dose_matrix(
            spec,
            case.voxel_locations_cm,
            case.voxel_structure,
            case.beam_of_beamlet,
            case.beamlet_u_cm,
            case.beamlet_z_cm,
            case.beamlet_width_cm,
            case.beamlet_height_cm,
            case.beam_energy_mv;
    ...
    if feasibility_status == feasible
        ...
        target_floor = min(
            target_floor, 0.99 * minimum(minimum(dose[target]) for dose in scenario_doses)
        )
        target_ceiling = max(
            target_ceiling, 1.01 * maximum(maximum(dose[target]) for dose in scenario_doses)
        )
        for structure in case.structure_names[2:end]
            indices = case.structure_voxels[structure]
            achieved = maximum(maximum(dose[indices]) for dose in scenario_doses)
            structure_max[structure] = max(structure_max[structure], 1.04 * achieved)
        end
    end
    ...
    fluence_penalty = (0.0012 + 0.0015rand(rng)) / length(case.reference_fluence)
    smoothness_penalty =
        isempty(case.beamlet_edges) ? 0.0 : (0.015 + 0.020rand(rng)) / length(case.beamlet_edges)
...
function _rt_robust_witness_is_valid(problem::RobustFluenceIMRTProblem; atol::Float64=1.0e-9)
    ...
    for matrix in problem.scenario_dose_matrices
        ...
        for (structure, upper) in problem.structure_max
            all(dose[case.structure_voxels[structure]] .<= upper + atol) || return false
        end
    end
    return true
end
...
function build_model(problem::RobustFluenceIMRTProblem)
    ...
    @objective(
        ...
        Min,
        sum(
                problem.underdose_weight[i] * underdose[i, scenario] for
                i in target, scenario in 1:n_scenarios
            ) / n_scenarios +
            sum(
                problem.overdose_weight[i] * overdose[i, scenario] for
                i in 1:n_voxels, scenario in 1:n_scenarios
            ) / n_scenarios +
