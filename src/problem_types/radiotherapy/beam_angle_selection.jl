...
function BeamAngleSelectionIMRTProblem(
    ...
    candidate_angles = profile == :breast ? collect(15.0:30.0:345.0) :
                                            collect(0.0:30.0:330.0)
    maximum_open = clamp(ceil(Int, 0.75 * length(base_spec.angles)), 4, 7)
    minimum_open = max(3, maximum_open - 2)
    reference_open = unique(round.(Int,
        range(1, length(candidate_angles); length=maximum_open)))
    case, spec, conflict_indices, rng = _rt_build_case(
        target_variables, :beam_angle_selection, feasibility_status, seed;
        profile_override=profile, angles=candidate_angles,
        active_beams=reference_open,
    )
    target_floor, target_ceiling, structure_max, base_witness, certificate =
        _rt_hard_limits(case, spec, feasibility_status, conflict_indices, rng)
    witness = base_witness === nothing ? nothing :
              RadiotherapyBeamSelectionWitness(
                  copy(base_witness.fluence), copy(reference_open),
              )

    target = case.structure_voxels[:ptv]
    peak_reference = maximum(case.reference_fluence)
    beamlet_fluence_max = fill(max(0.5, 2.5 * peak_reference),
                               length(case.reference_fluence))
    # Ensure the planted point remains strictly inside every linking row.
    beamlet_fluence_max = max.(beamlet_fluence_max,
                               1.05 .* case.reference_fluence)
    ...
    fluence_penalty = (0.0012 + 0.0018rand(rng)) /
                      length(case.reference_fluence)
    smoothness_penalty = isempty(case.beamlet_edges) ? 0.0 :
        (0.014 + 0.020rand(rng)) / length(case.beamlet_edges)
    return BeamAngleSelectionIMRTProblem(
        case, feasibility_status, minimum_open, maximum_open, reference_open,
        beamlet_fluence_max, target_floor, target_ceiling, structure_max,
        desired_dose, underdose_weight, overdose_weight, beam_open_penalty,
...
function _rt_beam_selection_witness_is_valid(
    ...
    problem.minimum_open_beams <= length(witness.open_beams) <=
        problem.maximum_open_beams || return false
...
function build_model(problem::BeamAngleSelectionIMRTProblem)
    ...
    @constraint(model, beamlet_link[j in 1:n_beamlets],
                fluence[j] <= problem.beamlet_fluence_max[j] *
                              beam_open[case.beam_of_beamlet[j]])
    @constraint(model, minimum_fields,
                sum(beam_open) >= problem.minimum_open_beams)
    @constraint(model, maximum_fields,
                sum(beam_open) <= problem.maximum_open_beams)
