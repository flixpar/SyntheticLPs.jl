...
function _rt_upper_tail_mean(values::AbstractVector{<:Real},
...
function _rt_lower_tail_mean(values::AbstractVector{<:Real},
                             weights::AbstractVector{<:Real},
                             fraction::Float64)
...
function MeanTailDoseIMRTProblem(
    ...
    unknown_tail_severity = feasibility_status == unknown ?
        exp(log(0.85) + rand(rng) * (log(2.80) - log(0.85))) : 1.0
    for structure in spec.structures
        indices = case.structure_voxels[structure]
        minimum_fraction = 1.0 / length(indices)
        fraction = clamp(spec.tail_fractions[structure] *
                         (0.92 + 0.16 * rand(rng)), minimum_fraction, 1.0)
        tail_fraction[structure] = fraction
        reference = case.reference_dose[indices]
        volume = case.voxel_volume_cc[indices]
        if structure == :ptv
            tail_bound[structure] = feasibility_status == unknown ?
                0.87 + 0.10 * rand(rng) :
                0.97 * _rt_lower_tail_mean(reference, volume, fraction)
            mean_dose_weight[structure] = 0.0
        else
            clinical = spec.clinical_caps[structure] *
                       (0.94 + 0.12 * rand(rng))
            tail_bound[structure] = feasibility_status == unknown ?
                clinical * unknown_tail_severity * (0.95 + 0.10 * rand(rng)) :
                max(clinical,
                    1.04 * _rt_upper_tail_mean(reference, volume, fraction))
            kind = kind_by_structure[structure]
            base_weight = kind == :serial_oar ? 3.5 :
                          kind == :parallel_oar ? 2.0 : 0.5
            mean_dose_weight[structure] = base_weight *
                                          (0.75 + 0.50 * rand(rng))
        end
    end
    fluence_penalty = (0.001 + 0.0015 * rand(rng)) /
                      length(case.reference_fluence)
    smoothness_penalty = isempty(case.beamlet_edges) ? 0.0 :
        (0.012 + 0.022 * rand(rng)) / length(case.beamlet_edges)

    return MeanTailDoseIMRTProblem(
        case, feasibility_status, target_floor, target_ceiling, structure_max,
        tail_fraction, tail_bound, mean_dose_weight, fluence_penalty,
        smoothness_penalty, witness, certificate,
    )
end

function _rt_mean_tail_witness_is_valid(problem::MeanTailDoseIMRTProblem;
                                        atol::Float64=1.0e-9)
    ...
    for structure in problem.case_data.structure_names
        ...
        volume = problem.case_data.voxel_volume_cc[
            problem.case_data.structure_voxels[structure]
        ]
        fraction = problem.tail_fraction[structure]
        if structure == :ptv
            _rt_lower_tail_mean(values, volume, fraction) >=
                problem.tail_bound[structure] - atol || return false
        else
            _rt_upper_tail_mean(values, volume, fraction) <=
                problem.tail_bound[structure] + atol || return false
        end
    end
    return true
...
function build_model(problem::MeanTailDoseIMRTProblem)
    ...
    for (s, structure) in enumerate(case.structure_names)
        ...
        if structure == :ptv
            ...
            tail_rows[structure] = @constraint(
                model,
                tail_threshold[s] -
                    sum(case.voxel_volume_cc[i] * tail_excess[i] for i in indices) /
                    tail_volume >= problem.tail_bound[structure],
    ...
    @objective(
        model,
        Min,
        sum(problem.mean_dose_weight[structure] /
            sum(case.voxel_volume_cc[i]
                for i in case.structure_voxels[structure]) *
            sum(case.voxel_volume_cc[i] * dose[i]
                for i in case.structure_voxels[structure])
    ...
    for e in 1:n_edges
        ...
        set_start_value(variation[e], abs(case.reference_fluence[a] -
                                          case.reference_fluence[b]))
    end
    return model
end
