...
function WeightedDeviationIMRTProblem(
    ...
    for structure in spec.structures
        ...
        if kind == :target
            ...
            base_weight = kind == :serial_oar ? 18.0 :
                          kind == :parallel_oar ? 8.0 : 1.2
            overdose_weight[indices] .= base_weight *
                                        (0.75 + 0.50 * rand(rng)) .* volume_share
        end
    end
    fluence_penalty = (0.0015 + 0.0020 * rand(rng)) /
                      length(case.reference_fluence)
    smoothness_penalty = isempty(case.beamlet_edges) ? 0.0 :
        (0.018 + 0.025 * rand(rng)) / length(case.beamlet_edges)

    return WeightedDeviationIMRTProblem(
        case, feasibility_status, target_floor, target_ceiling, structure_max,
        desired_dose, underdose_weight, overdose_weight, fluence_penalty,
...
function build_model(problem::WeightedDeviationIMRTProblem)
    ...
    for i in target
        set_start_value(underdose[i], max(0.0, problem.desired_dose[i] -
                                               reference_dose[i]))
    end
    for i in 1:n_voxels
        set_start_value(overdose[i], max(0.0, reference_dose[i] -
                                              problem.desired_dose[i]))
    end
    for e in 1:n_edges
