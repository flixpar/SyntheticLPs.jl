...
function BudgetedBackboneHubProblem(
    ...
    fixed_cost = [total_flow * average_distance * rand(rng, Uniform(0.015, 0.055)) for _ in 1:h]
    ...
    for k in 1:h, m in (k + 1):h
        ...
        cost =
            total_flow *
            (0.006 + 0.018 * d / max(average_distance, 1.0)) *
    ...
    if feasibility_status == feasible
        ...
        for i in 1:n
            ...
            assignment[i] = own in chosen ? own : chosen[argmin([dist[i, hubs[k]] for k in chosen])]
        end
        links = [(k, m) for (pos, k) in enumerate(chosen) for m in chosen[(pos + 1):end]]
        for (k, m) in links
            required = sum(
                flow[i, j] for i in 1:n, j in 1:n if (assignment[i] == k && assignment[j] == m) ||
                    (assignment[i] == m && assignment[j] == k);
                init=0.0,
            )
            link_capacity[k, m] =
                link_capacity[m, k] = max(
                    link_capacity[k, m], required * rand(rng, Uniform(1.08, 1.25))
                )
        end
        link_budget = sum(link_cost[k, m] for (k, m) in links) * rand(rng, Uniform(1.03, 1.15))
...
function build_model(prob::BudgetedBackboneHubProblem)
    ...
    @constraint(
        model,
        active_hub_degree[k = 1:h],
        sum(b[min(k, m), max(k, m)] for m in 1:h if m != k) >= y[k]
    )
    @constraint(
        model,
        link_budget,
        sum(prob.link_cost[k, m] * b[k, m] for k in 1:h, m in (k + 1):h) <= prob.link_budget
    )
    @constraint(
        model,
        flow_balance[i = 1:n, k = 1:h],
        sum(q[i, k, m] for m in 1:h if m != k) - sum(q[i, m, k] for m in 1:h if m != k) ==
            prob.outvolume[i] * z[i, k] - sum(prob.flow[i, j] * z[j, k] for j in 1:n)
    )
    @constraint(
        model,
        physical_link_capacity[k = 1:h, m = 1:h; k < m],
        sum(q[i, k, m] + q[i, m, k] for i in 1:n) <= prob.link_capacity[k, m] * b[k, m]
    )
    ...
    @objective(
        ...
        sum(prob.fixed_cost[k] * y[k] for k in 1:h) +
            ...
            sum(
                prob.chi * prob.dist[i, prob.hubs[k]] * prob.outvolume[i] * z[i, k] for
                i in 1:n, k in 1:h
            ) +
            sum(
                prob.delta * prob.dist[prob.hubs[k], j] * prob.involume[j] * z[j, k] for
                j in 1:n, k in 1:h
            ) +
            sum(
                prob.alpha * prob.dist[prob.hubs[k], prob.hubs[m]] * q[i, k, m] for
                i in 1:n, k in 1:h, m in 1:h if k != m
            )
    )
    return model
end
