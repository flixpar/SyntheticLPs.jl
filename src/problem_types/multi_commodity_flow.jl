...
"""
    generate_multi_commodity_flow_problem(params::Dict=Dict(); seed::Int=0)

Generate a multi-commodity flow problem instance.
...
- `params`: Dictionary of problem parameters
  - `:n_nodes`: Number of nodes in the network (default: 10)
  - `:n_arcs`: Number of arcs in the network (default: 30)
  - `:n_commodities`: Number of commodities (default: 3)
  - `:capacity_range`: Tuple (min, max) for arc capacities (default: (50.0, 500.0))
  - `:demand_range`: Tuple (min, max) for commodity demands (default: (10.0, 100.0))
  - `:cost_range`: Tuple (min, max) for arc costs (default: (1.0, 20.0))
  - `:capacity_utilization`: Target utilization of network capacity (default: 0.7)
...
# Details
The problem models scenarios such as:
- Telecommunications: routing different message types through a network
- Supply chain: shipping multiple product types through distribution network
- Transportation: moving different freight classes through shared infrastructure
"""
function generate_multi_commodity_flow_problem(params::Dict=Dict(); seed::Int=0)
    # Set random seed
    Random.seed!(seed)

    # Extract parameters with defaults
    n_nodes = get(params, :n_nodes, 10)
    n_arcs = get(params, :n_arcs, 30)
    n_commodities = get(params, :n_commodities, 3)
    capacity_range = get(params, :capacity_range, (50.0, 500.0))
    demand_range = get(params, :demand_range, (10.0, 100.0))
    cost_range = get(params, :cost_range, (1.0, 20.0))
    capacity_utilization = get(params, :capacity_utilization, 0.7)
    ...
    actual_params[:total_demand] = sum(values(demands))
    actual_params[:total_capacity] = sum(values(capacities))

    # Create model
    model = Model()

    # Decision variables: flow of each commodity on each arc
    @variable(model, flow[1:n_commodities, arc in arcs] >= 0)

    # Objective: Minimize total cost
    @objective(model, Min,
        sum(costs[arc] * sum(flow[k, arc] for k in 1:n_commodities) for arc in arcs)
    )

    # Capacity constraints: total flow on each arc cannot exceed capacity
    for arc in arcs
        @constraint(model,
            sum(flow[k, arc] for k in 1:n_commodities) <= capacities[arc]
        )
    end
    ...
    for k in 1:n_commodities
        ...
        for node in 1:n_nodes
            ...
            inflow_arcs = [arc for arc in arcs if arc[2] == node]
            # Arcs leaving this node
            outflow_arcs = [arc for arc in arcs if arc[1] == node]
            ...
            if node == source
                ...
                @constraint(model, outflow - inflow == demands[k])
            elseif node == sink
                # At sink: inflow - outflow = demand
                @constraint(model, inflow - outflow == demands[k])
            else
...
"""
    calculate_multi_commodity_flow_variable_count(params::Dict)

Calculate the total number of variables for a multi-commodity flow problem.

# Arguments
- `params`: Dictionary of problem parameters containing :n_commodities and :n_arcs

# Returns
- Total number of variables in the problem
"""
function calculate_multi_commodity_flow_variable_count(params::Dict)
    # Extract parameters with defaults
    n_commodities = get(params, :n_commodities, 3)
    n_arcs = get(params, :n_arcs, 30)

    # Variables:
    # - flow[k, arc] for each commodity k and each arc: n_commodities × n_arcs
    # Total: n_commodities × n_arcs

    return n_commodities * n_arcs
end

"""
    sample_multi_commodity_flow_parameters(target_variables::Int; seed::Int=0)
...
"""
function sample_multi_commodity_flow_parameters(target_variables::Int; seed::Int=0)
    Random.seed!(seed)
...
"""
    sample_multi_commodity_flow_parameters(size::Symbol; seed::Int=0)

Legacy function for backward compatibility. Sample realistic parameters for a
multi-commodity flow problem using size categories.
...
- Dictionary of sampled parameters
"""
function sample_multi_commodity_flow_parameters(size::Symbol; seed::Int=0)
    Random.seed!(seed)
    ...
    return sample_multi_commodity_flow_parameters(target_map[size]; seed=seed)
end

# Register the problem type
register_problem(
    :multi_commodity_flow,
    generate_multi_commodity_flow_problem,
    sample_multi_commodity_flow_parameters,
