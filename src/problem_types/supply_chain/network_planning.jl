...
const _NETWORK_PLANNING_PROFILES =
    ...
    for n_periods in _network_period_range(profile), n_products in 2:5, n_plants in 2:20
        ...
        for n_customers in customer_candidates
            ...
            if profile == :disruption
                ...
                max_arcs -= n_customers * n_products *
                            max(0, max_degree - min(max_degree, n_plants - 1))
            end
            arc_budget = clamp(target - dense_vars, demand_nodes, max_arcs)
            delivered = dense_vars + arc_budget
            size_error = abs(delivered - target)
            density_error =
                abs(arc_budget / (n_plants * demand_nodes) - density_mid)
...
function _network_locations(rng::AbstractRNG, n_plants::Int, n_customers::Int)
    ...
    centers = [(rand(rng, Uniform(10.0, 90.0)),
                rand(rng, Uniform(10.0, 90.0))) for _ in 1:n_regions]
...
function _network_specialization(rng::AbstractRNG, n_plants::Int, n_products::Int)
    ...
    resource_use = [product_bulk[k] / sqrt(specialization[p, k])
...
function _network_demand(
    ...
    if profile == :regional_stable
        ...
        seasonal .= [1 + 0.07 * sin(2π * (t - 1) / n_periods + phase)
                     for t in 1:n_periods]
    elseif profile == :seasonal_prebuild
        peak = max(3, ceil(Int, 0.72 * n_periods))
        seasonal .= [0.63 + 1.12 * exp(-0.5 * ((t - peak) / 1.05)^2)
                     for t in 1:n_periods]
    else
    ...
    for c in 1:n_customers, k in 1:n_products, t in 1:n_periods
        demand[c, k, t] = customer_scale[c] * product_scale[k] * seasonal[t] *
...
function _network_arcs(
    ...
    for c in 1:n_customers, k in 1:n_products, t in 1:n_periods
        node = (c, k, t)
        eligible = [p for p in 1:n_plants
        ...
        for p in eligible
            ...
            region_penalty = plant_regions[p] == customer_regions[c] ? 0.0 :
                             (profile == :regional_stable ? 28.0 : 12.0)
            specialist_penalty = 24.0 / specialization[p, k]
            time_jitter = rand(rng, Uniform(0, 4))
            push!(scored, (distance + region_penalty + specialist_penalty +
                           time_jitter, p))
        end
        sort!(scored)
    ...
    for (_, arc) in extras
        ...
        _, c, k, t = arc
        node = (c, k, t)
        period_max = profile == :disruption && t == disruption_period ?
                     min(max_degree, n_plants - 1) : max_degree
...
function _network_reference_plan(
    ...
    for c in 1:n_customers, k in 1:n_products, t in 1:n_periods
        ...
        for arc in arcs
            ...
            distance = _network_distance(plant_locations[p], customer_locations[c])
            disruption_bias =
                profile == :disruption && t == disruption_period ? 0.8 : 1.0
            push!(weights, disruption_bias * specialization[p, k] /
                           (10 + distance) * rand(rng, Uniform(0.9, 1.1)))
        end
        weights ./= sum(weights)
    ...
    for p in 1:n_plants, k in 1:n_products
        ...
        for t in 1:n_periods
            desired = if profile == :seasonal_prebuild
                peak = max(3, ceil(Int, 0.72 * n_periods))
                t < peak ? 0.20 * total_out * t / max(peak - 1, 1) :
                0.20 * total_out * max(n_periods - t, 0) /
                max(n_periods - peak + 1, 1)
            elseif profile == :disruption
                t < disruption_period ?
                0.12 * total_out * t / max(disruption_period - 1, 1) :
                0.04 * total_out * max(n_periods - t, 0) /
                max(n_periods - disruption_period + 1, 1)
            else
    ...
    return initial_inventory, witness_production, witness_inventory,
           witness_shipment
end
...
function SupplyChainNetworkPlanningProblem(
    ...
    n_plants, n_customers, n_products, n_periods, arc_budget =
        _choose_network_planning_dimensions(target_variables, profile)
    ...
    specialization, resource_use =
        _network_specialization(rng, n_plants, n_products)
    demand = _network_demand(rng, profile, n_customers, n_products, n_periods,
                             disruption_period)
    shipment_arcs = _network_arcs(
        rng, profile, arc_budget, n_plants, n_customers, n_products, n_periods,
        plant_locations, customer_locations, plant_regions, customer_regions,
        specialization, disruption_period, disrupted_plant,
    )
    initial_inventory, witness_production, witness_inventory, witness_shipment =
        _network_reference_plan(
            rng, profile, demand, shipment_arcs, plant_locations,
            customer_locations, specialization, n_plants, n_customers,
            n_products, n_periods, disruption_period,
        )
    ...
    for p in 1:n_plants, k in 1:n_products, t in 1:n_periods
        time_factor = profile == :seasonal_prebuild ?
                      0.82 + 0.09 * (t - 1) : 0.96 + 0.025 * (t - 1)
        if profile == :disruption && t == disruption_period &&
           p == disrupted_plant
            time_factor *= 1.8
        end
        production_cost[p, k, t] =
            rand(rng, LogNormal(log(24.0), 0.18)) * time_factor /
            specialization[p, k]
        holding_cost[p, k, t] =
            rand(rng, Uniform(0.35, 1.3)) * (1 + 0.05 * (t - 1))
    end
    ...
    for arc in shipment_arcs
        ...
        if profile == :disruption && t == disruption_period
            ...
            lane_capacity[arc] =
                max(lane_capacity[arc], 1.06 * witness_shipment[arc])
        end
    end
    ...
    for p in 1:n_plants, t in 1:n_periods
        used = sum(resource_use[p, k] * witness_production[p, k, t]
                   for k in 1:n_products)
        plant_capacity[p, t] = used * rand(rng, Uniform(1.10, 1.25)) +
                               0.02 * sum(demand[:, :, t])
        if profile == :disruption && t == disruption_period &&
           p == disrupted_plant
    ...
    for p in 1:n_plants, k in 1:n_products
        ...
        average_out = sum((witness_shipment[a] for a in shipment_arcs
                           if a[1] == p && a[3] == k); init=0.0) / n_periods
        inventory_capacity[p, k] = max(
            1.18 * peak_inventory + 0.08 * average_out + 1.0,
            1.10 * initial_inventory[p, k],
        )
    end
    ...
    if feasibility_status == infeasible
        ...
        certificate_period = profile == :disruption ? disruption_period :
                             rand(rng, max(1, n_periods - 2):n_periods)
        ...
        production_upper = [
            min(production_capacity[p, k, t],
                plant_capacity[p, t] / resource_use[p, k])
            for p in 1:n_plants, t in 1:tau
        ]
        raw_production = sum(production_upper)
        scale = clamp((desired_supply - initial_total) /
                      max(raw_production, eps()), 0.0, 0.95)
        for p in 1:n_plants, t in 1:tau
            ...
            production_capacity[p, k, t] =
                scale * production_upper[p, t]
        end

        certificate_supply_bound = sum(initial_inventory[:, k]) + sum(
            min(production_capacity[p, k, t],
                plant_capacity[p, t] / resource_use[p, k])
            for p in 1:n_plants, t in 1:tau
        )
        certificate_lane_bound = sum(
            lane_capacity[a] for a in shipment_arcs
            if a[3] == k && a[4] <= tau
        )
...
function build_model(prob::SupplyChainNetworkPlanningProblem)
    model = Model()
    P, C, K, T =
        prob.n_plants, prob.n_customers, prob.n_products, prob.n_periods
    arcs = prob.shipment_arcs

    @variable(
        model,
        0 <= produce[p=1:P, k=1:K, t=1:T] <=
             prob.production_capacity[p, k, t]
    )
    @variable(
        model,
        0 <= inventory[p=1:P, k=1:K, t=1:T] <=
             prob.inventory_capacity[p, k]
    )
    ...
    @objective(
        model,
        Min,
        sum(prob.production_cost[p, k, t] * produce[p, k, t]
            for p in 1:P, k in 1:K, t in 1:T) +
        sum(prob.holding_cost[p, k, t] * inventory[p, k, t]
            for p in 1:P, k in 1:K, t in 1:T) +
