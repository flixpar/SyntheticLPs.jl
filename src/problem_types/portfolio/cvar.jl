...
function PortfolioProblem(target_variables::Int, feasibility_status::FeasibilityStatus, seed::Int)
    ...
    if target_variables <= 100
        ...
        n_regions = round(Int, rand(rng, Uniform(3, 4)))
        n_asset_classes = round(Int, rand(rng, Uniform(3, 4)))
        n_factors = round(Int, rand(rng, Uniform(3, 5)))
    elseif target_variables <= 500
        n_sectors = round(Int, rand(rng, Uniform(5, 10)))
        n_regions = round(Int, rand(rng, Uniform(3, 5)))
        n_asset_classes = round(Int, rand(rng, Uniform(3, 5)))
        n_factors = round(Int, rand(rng, Uniform(4, 7)))
    else
        n_sectors = round(Int, rand(rng, Uniform(8, 12)))
        n_regions = round(Int, rand(rng, Uniform(4, 6)))
        n_asset_classes = round(Int, rand(rng, Uniform(4, 5)))
    ...
    # --- CVaR parameters ---
    beta = rand(rng, Uniform(0.90, 0.99))
    ...
    cvar_limit = ref_cvar * rand(rng, Uniform(0.8, 1.5))
    ...
    factor_lower = ref_exposure .- rand(rng, Uniform(0.1, 0.3), n_factors)
    factor_upper = ref_exposure .+ rand(rng, Uniform(0.1, 0.3), n_factors)
    ...
    max_position = [(2.0 + 3.0 * rand(rng)) / n_assets for _ in 1:n_assets]
    ...
    # --- Turnover limit ---
    turnover_limit = rand(rng, Uniform(0.3, 1.5))
    ...
    if actual_status == feasible
        ...
        for s in 1:n_sectors
            ...
            sector_upper[s] = max(sector_upper[s], sw * (1.1 + 0.2 * rand(rng)))
        ...
        for r in 1:n_regions
            ...
            region_upper[r] = max(region_upper[r], rw * (1.1 + 0.2 * rand(rng)))
        ...
        for c in 1:n_asset_classes
            ...
            asset_class_lower[c] = min(asset_class_lower[c], cw * (0.8 - 0.1 * rand(rng)))
            asset_class_upper[c] = max(asset_class_upper[c], cw * (1.2 + 0.2 * rand(rng)))
        ...
        for f in 1:n_factors
            factor_lower[f] = min(factor_lower[f], ref_fexp[f] - rand(rng, Uniform(0.05, 0.15)))
            factor_upper[f] = max(factor_upper[f], ref_fexp[f] + rand(rng, Uniform(0.05, 0.15)))
        end
        ...
        cvar_limit = max(cvar_limit, ref_cvar_val * (1.05 + 0.15 * rand(rng)))

        # Ensure turnover limit accommodates reference
        ref_turnover = sum(abs.(ref_portfolio .- benchmark))
        turnover_limit = max(turnover_limit, ref_turnover * (1.1 + 0.2 * rand(rng)))
