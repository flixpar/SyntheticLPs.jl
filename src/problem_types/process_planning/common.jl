...
function _pp_realize(structure::PPStructure, n_crudes::Int)
    ...
    # Every refinery stream has an outside market — naphtha, gasoil and VGO
    # cargoes, LPG, refinery fuel, petcoke, bunker cutter stock — so each stream
    # can be sold as it is, at a discount to what it is worth in a blend and
    # against a limited spot volume. Purchased blendstocks are excluded so the
    # model cannot buy and resell the same barrel.
    spot = [s for s in eachindex(stream_classes) if !(s in purchasable)]
...
"""
    _pp_dimensions(rng, target; mode_vars) -> (structure, skeleton, n_periods)
...
function _pp_dimensions(rng::AbstractRNG, target::Int; mode_vars::Bool)
    ...
    best_score = (Inf, Inf)
    for level in 1:length(_PP_LEVEL_UNITS)
        structure = _pp_structure(rng, target, level; single_mode=!mode_vars)
        ...
        v2 = _pp_variables_per_period(_pp_realize(structure, 2); mode_vars=mode_vars)
        v3 = _pp_variables_per_period(_pp_realize(structure, 3); mode_vars=mode_vars)
        slope = max(v3 - v2, 1)
...
Quality vector of one blend. Volumetric properties are volume-weighted; sulfur is
...
"""
    _pp_market_path(rng, T, base; volatility, seasonality, phase)
...
function _pp_market_path(rng::AbstractRNG, T::Int, base::Real;
    ...
    for t in 1:T
        ...
        season = seasonality * sin(2pi * (t - 1) / max(T, 2) + phase)
...
function _pp_plan_instance(rng::AbstractRNG, fs::RefineryFlowsheet, T::Int,
    ...
    for c in 1:C
        ...
        crude_price[c, :] .= _pp_market_path(rng, T, base * rand(rng, Uniform(0.95, 1.06));
                                             volatility=0.045, seasonality=0.02,
    ...
    for u in 1:U
        ...
        for t in 1:T
            ...
            unit_capacity[u, t] = planned ?
                max(design, required * rand(rng, Uniform(1.05, 1.40))) :
                design * rand(rng, Uniform(0.88, 1.15))
    ...
    for s in 1:S
        ...
        if s in fs.purchasable
            ...
            stream_purchase_cost[s, :] .=
                _pp_market_path(rng, T, base * rand(rng, Uniform(0.92, 1.10));
                                volatility=0.05, seasonality=0.03)
    ...
    for p in 1:P
        ...
        phase = _pp_demand_phase(fs.products[p].key)
        product_price[p, :] .=
            _pp_market_path(rng, T, template.price * rand(rng, Uniform(0.94, 1.08));
                            volatility=0.05, seasonality=0.05, phase=phase)
        ...
        for t in 1:T
            ...
            if planned
                ...
                demand_min[p, t] = sales * market[t] * rand(rng, Uniform(0.65, 1.55))
    ...
    if status == feasible
        ...
        _pp_settle_specifications!(rng, fs, plan; slack_low=-0.03, slack_high=0.06)
    end
