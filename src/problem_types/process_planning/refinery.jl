...
of "the volume-weighted average stays below `s`"; a weight-basis property (sulfur)
carries the component density, giving the mass-weighted average.
    ...
    for (p, product) in enumerate(fs.products)
        for q in 1:PP_N_QUALITIES
            ...
            for (bound, sense) in ((product.spec_min[q], :min),
                ...
                coefficient = [weight[b] * (fs.qualities[s, q] - bound)
