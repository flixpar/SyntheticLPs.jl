...
- `initial_mode::Vector{Int}`: the mode each unit ran before the horizon opened
  ...
  measured against
- `minimum_run::Vector{Int}`: minimum periods a newly started mode must remain on
...
Plant a seasonal campaign: each multi-mode unit runs its first mode for one long
block and, when the horizon permits, its second for the remainder — a cracker
...
function _pp_campaign_modes(
    ...
    for u in 1:U
        ...
        L = minimum_run[u]
        if T >= 2L
            split = rand(rng, L:(T - L))
            modes[u, (split + 1):T] .= alternate
        end
        # Two long seasonal campaigns are preferable to a one-period third-mode
        # spike: every planted transition must honour the same minimum-run rule
        # imposed on the optimization model.
    end
    return modes
end
...
function RefineryModeSwitchingProblem(
    ...
    if feasibility_status == feasible
        ...
        @assert _pp_mode_schedule_satisfies(mode_choice, initial_mode, minimum_run)
...
the at-most-one-mode rule, the three standard inequalities that make
`switch = max(0, z_t - z_{t-1})` exact, and horizon-safe minimum-run constraints
following every start.
...
function build_model(prob::RefineryModeSwitchingProblem)
    ...
    model[:run_mode] = run_mode
    model[:mode_start] = switch
