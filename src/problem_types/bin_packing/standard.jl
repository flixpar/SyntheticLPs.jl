    ...
    for n_items in 3:maximum_items
        ...
        for n_bins in minimum_bins:maximum_bins
            ...
            for n_categories in 2:maximum_categories
                actual = _bin_packing_variable_count(n_items, n_bins, n_categories)
                realism_penalty =
...
function _packing_category_data(rng::AbstractRNG, n_items::Int, n_categories::Int)
    categories = [mod(item - 1, n_categories) + 1 for item in 1:n_items]
    shuffle!(rng, categories)
    names = [String(_PACKING_CATEGORY_PROFILES[category].name) for category in 1:n_categories]

    candidates = [
        pair for
        pair in _PACKING_CONFLICT_CANDIDATES if pair[1] <= n_categories && pair[2] <= n_categories
    ]
...
function _sample_packing_sizes(rng::AbstractRNG, item_categories::Vector{Int}, capacity::Float64)
    ...
    for item in eachindex(item_categories)
        ...
        if rand(rng) < 0.55
            ...
            fraction = round(fraction / 0.05) * 0.05 + rand(rng, Normal(0.0, 0.0075))
        end
...
function _packing_conflict_set(incompatible_pairs::Vector{Tuple{Int, Int}})
    return Set(
        (min(first(pair), last(pair)), max(first(pair), last(pair))) for pair in incompatible_pairs
    )
end

function _first_fit_conflict_packing(
    item_sizes::Vector{Float64},
    capacity::Float64,
    item_categories::Vector{Int},
    incompatible_pairs::Vector{Tuple{Int, Int}},
    ...
    order = sortperm(eachindex(item_sizes); by=item -> (-item_sizes[item], item))
...
function _fit_standard_packing!(
    item_sizes::Vector{Float64},
    capacity::Float64,
    item_categories::Vector{Int},
    incompatible_pairs::Vector{Tuple{Int, Int}},
...
function _bounded_sizes_with_total(
    base_sizes::Vector{Float64}, target::Float64, upper_bounds::Vector{Float64}
)
...
function _force_standard_overload!(
    rng::AbstractRNG,
    item_sizes::Vector{Float64},
    n_bins::Int,
    capacity::Float64;
...
function _set_bin_packing_starts!(model::Model, prob)
    ...
    for item in 1:prob.n_items
        ...
        for candidate_bin in 1:prob.n_bins
            set_start_value(model[:x][item, candidate_bin], candidate_bin == bin ? 1.0 : 0.0)
        end
    end
    for bin in 1:prob.n_bins
        ...
        for category in 1:prob.n_categories
            set_start_value(
                model[:category_present][category, bin], present[category, bin] ? 1.0 : 0.0
            )
        end
    end
    return model
...
function BinPackingProblem(target_variables::Int, feasibility_status::FeasibilityStatus, seed::Int)
    ...
    bin_capacity = if n_items <= 25
        rand(rng, Uniform(80.0, 140.0))
    elseif n_items <= 100
        rand(rng, Uniform(300.0, 650.0))
    else
    ...
    if feasibility_status == feasible
        bins = _fit_standard_packing!(
            item_sizes, bin_capacity, item_categories, incompatible_pairs, n_bins
        )
        feasible_witness = _canonical_bin_assignment(bins, n_items)
    elseif feasibility_status == infeasible
        aggregate_capacity = _force_standard_overload!(rng, item_sizes, n_bins, bin_capacity)
    ...
    elseif load_profile in (:light, :nominal)
        ...
        _force_standard_overload!(rng, item_sizes, n_bins, bin_capacity; factor_bounds=(1.03, 1.09))
    end
...
category's total assignment. The chain `x[i,b] <= category_present[c(i),b] <= y[b]` makes separate item-to-used-bin rows unnecessary even in the relaxation.
...
function build_model(prob::BinPackingProblem)
    ...
    @constraint(
        ...
        sum(prob.item_sizes[item] * x[item, bin] for item in I) <= prob.bin_capacity * y[bin]
    )
    ...
    # representation.
    if prob.n_bins >= 2
        @constraint(model, used_prefix[bin in 1:(prob.n_bins - 1)], y[bin] >= y[bin + 1])
    end
    @constraint(model, canonical_label[item in I, bin in B; bin > item], x[item, bin] == 0)
