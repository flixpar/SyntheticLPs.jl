...
function _heterogeneous_greedy_packing(
    ...
    eligible_slot_count(item) =
        count(compatibility[bin_types[bin], item_categories[item]] for bin in 1:n_bins)
    order = sortperm(
        eachindex(item_sizes); by=item -> (eligible_slot_count(item), -item_sizes[item], item)
    )
    ...
    for item in order
        ...
        for bin in 1:n_bins
            ...
            conflict = any(
                (min(category, other), max(category, other)) in conflicts for
                other in bin_categories[bin] if other != category
            )
            conflict && continue
...
function _fit_heterogeneous_packing!(
    ...
    maximum_utilization = maximum(
        loads[bin] / type_capacities[bin_types[bin]] for bin in eachindex(bin_types)
    )
...
_heterogeneous_aggregate_capacity(bin_types::Vector{Int}, type_capacities::Vector{Float64}) =
    sum(type_capacities[bin_type] for bin_type in bin_types)

function _heterogeneous_size_upper_bounds(
    item_categories::Vector{Int}, type_capacities::Vector{Float64}, compatibility::BitMatrix
)
    return [
        0.86 * maximum(
            type_capacities[bin_type] for
            bin_type in eachindex(type_capacities) if compatibility[bin_type, item_categories[item]]
...
function _force_heterogeneous_overload!(
    ...
    aggregate_capacity = _heterogeneous_aggregate_capacity(bin_types, type_capacities)
    upper_bounds = _heterogeneous_size_upper_bounds(item_categories, type_capacities, compatibility)
    ...
    item_sizes .= _bounded_sizes_with_total(item_sizes, total_size, upper_bounds)
...
function _canonicalize_heterogeneous_assignment(
    assignment::Vector{Int}, bin_types::Vector{Int}, n_bin_types::Int
)
    canonical = copy(assignment)
    for bin_type in 1:n_bin_types
        type_slots = findall(==(bin_type), bin_types)
        used_slots = sort(unique(bin for bin in assignment if bin_types[bin] == bin_type))
        mapping = Dict(used_slots[index] => type_slots[index] for index in eachindex(used_slots))
...
function validate_bin_packing_witness(prob::HeterogeneousBinPackingProblem)
    ...
    count_by_type = [count(==(bin_type), prob.bin_types) for bin_type in 1:prob.n_bin_types]
    ...
    for bin in sort(unique(witness))
        ...
        all(
            prob.type_category_compatibility[bin_type, prob.item_categories[item]] for item in items
        ) || return false
        ...
        for first_index in 1:length(categories)
            for second_index in (first_index + 1):length(categories)
                ...
                pair = (min(first_category, second_category), max(first_category, second_category))
...
function validate_bin_packing_certificate(prob::HeterogeneousBinPackingProblem)
    ...
    all(bin_type -> 1 <= bin_type <= prob.n_bin_types, prob.bin_types) || return false
    observed_availability = [count(==(bin_type), prob.bin_types) for bin_type in 1:prob.n_bin_types]
    observed_availability == prob.type_availability || return false
    demand = sum(prob.item_sizes)
    capacity = _heterogeneous_aggregate_capacity(prob.bin_types, prob.type_capacities)
...
function HeterogeneousBinPackingProblem(
    ...
    elseif n_items <= 100
        rand(rng, Uniform(320.0, 700.0))
    else
        rand(rng, Uniform(950.0, 2_100.0))
    end

    item_categories, category_names, incompatible_pairs = _packing_category_data(
        rng, n_items, n_categories
    )
    item_sizes = _sample_packing_sizes(rng, item_categories, base_capacity)
    bin_type_names, type_capacities, type_costs, type_availability, bin_types, compatibility = _heterogeneous_type_data(
        rng, n_bins, n_categories, base_capacity
    )
    n_bin_types = length(bin_type_names)
    load_profile = if feasibility_status == feasible
        :guaranteed_feasible
    ...
    elseif feasibility_status == infeasible
        aggregate_capacity = _force_heterogeneous_overload!(
            rng, item_sizes, item_categories, bin_types, type_capacities, compatibility
        )
        infeasibility_certificate = BinPackingCapacityCertificate(
            sum(item_sizes), aggregate_capacity, sum(item_sizes) - aggregate_capacity
        )
...
function build_model(prob::HeterogeneousBinPackingProblem)
    ...
    for bin_type in 1:prob.n_bin_types
        slots = findall(==(bin_type), prob.bin_types)
        append!(
            type_prefix_pairs, [(slots[index], slots[index + 1]) for index in 1:(length(slots) - 1)]
        )
    end
    @constraint(
        model,
        used_type_prefix[index in eachindex(type_prefix_pairs)],
