...
This variant is an LP: the commitment, startup, and shutdown variables (`on`,
`startup`, `shutdown`) are LP-relaxed to the continuous interval `[0, 1]` rather
than being binary. The formulation otherwise matches a standard MILP unit
commitment model, so it yields the LP relaxation of that problem.
...
For `feasible`, demand and reserve are scaled (and capacity bumped) so that
available capacity exceeds demand+reserve with a margin in every period. This is
a capacity-margin heuristic: it makes a feasible dispatch overwhelmingly likely
but is not a formal proof, since ramp / min up-down logic could in principle
bind. For `infeasible`, demand+reserve is forced to exceed available capacity by
at least ~10% in at least one period, which deterministically violates that
period's demand-balance constraint. For `unknown`, a natural instance is
generated by randomly selecting one of the two scenarios.
"""
function UnitCommitmentProblem(target_variables::Int, feasibility_status::FeasibilityStatus, seed::Int)
    ...
    scale = target_variables < 200 ? :tiny : target_variables < 800 ? :small :
            target_variables < 3000 ? :medium : :large
    ...
    runif(r) = r[1] < r[2] ? rand(Uniform(r[1], r[2])) : float(r[1])
    ...
    if actual_status == feasible
        # Capacity-margin heuristic: shrink demand/reserve (and, if needed, grow
        # capacity) so available capacity exceeds demand+reserve with a margin in
        # every period. This makes a feasible dispatch overwhelmingly likely.
    ...
    if actual_status == feasible
        # Final guard: keep demand+reserve safely below capacity in every period.
        max_ratio = maximum(ratios)
        ...
        # Final guard: force demand+reserve to exceed capacity by >=10% somewhere,
        # which makes the demand-balance constraint provably violable -> infeasible.
        max_ratio = maximum(ratios)
        if max_ratio < 1.1
...
Build a JuMP model for the unit commitment problem. Deterministic — uses only data
from the struct fields. Commitment, startup, and shutdown variables are LP-relaxed
to the continuous interval `[0, 1]`.
...
register_variant(
    ...
    "Unit commitment power system planning (LP relaxation) with ramping, startup/shutdown, reserves, and realistic generator portfolios",
)
