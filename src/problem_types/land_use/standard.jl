...
Base.:(==)(a::LandUseInfeasibilityCertificate,
           b::LandUseInfeasibilityCertificate) =
    a.resource_index == b.resource_index &&
    a.per_parcel_minimum == b.per_parcel_minimum &&
    a.lower_bound == b.lower_bound && a.capacity == b.capacity
Base.isequal(a::LandUseInfeasibilityCertificate,
             b::LandUseInfeasibilityCertificate) = a == b
...
function _land_use_minimum_counts(n_parcels::Int, n_zoning_types::Int,
                                  enabled::Bool)
    enabled || return Int[]
...
function _land_use_reference_assignment(net_benefit::Matrix{Float64},
    ...
    if residential_minimum + industrial_minimum > 0
        ...
        compatible_pool = length(odd_parcels) >= length(even_parcels) ?
                          odd_parcels : even_parcels
        ...
        residential_order = sort(compatible_pool;
            by = i -> (coordinates[i, 1], -coordinates[i, 2]))
        ...
        industrial_order = sort(remaining_compatible;
            by = i -> (-coordinates[i, 1], coordinates[i, 2]))
    ...
    if commercial_minimum > 0
        ...
        sort!(available; by = i ->
              (coordinates[i, 1] - 0.5)^2 + (coordinates[i, 2] - 0.5)^2)
    ...
    for parcel in 1:n_parcels
        ...
        for zoning in zoning_order
            if zoning == 1 && any(assignment[neighbor] == 3
                                  for neighbor in neighbors[parcel])
                continue
            elseif zoning == 3 && any(assignment[neighbor] == 1
                                      for neighbor in neighbors[parcel])
                continue
            end
            selected = zoning
...
"""
function LandUseProblem(target_variables::Int, feasibility_status::FeasibilityStatus,
                        seed::Int)
    ...
    zoning_names = [String(_LAND_USE_ZONING_CATALOG[j].name)
                    for j in 1:n_zoning_types]
    resource_names = [String(_LAND_USE_RESOURCE_NAMES[k])
                      for k in 1:n_resources]

    parcel_coordinates, adjacency_matrix, adjacency_edges, parity =
        _land_use_spatial_graph(rng, n_parcels)
    ...
    for parcel in 1:n_parcels
        ...
        for zoning in 1:n_zoning_types
            ...
            urban_weight = zoning in (1, 2, 3, 6, 8, 9, 10, 11) ?
                           accessibility : rurality
            development_costs[parcel, zoning] =
                development_cost_scale * profile.cost *
                (0.70 + 0.65 * urban_weight) * rand(rng, LogNormal(0.0, 0.18))
            revenues[parcel, zoning] =
                revenue_scale * profile.revenue *
                (0.55 + 1.05 * urban_weight) * rand(rng, LogNormal(0.0, 0.22))
        end
    end
    ...
    for zoning in 1:n_zoning_types, resource in 1:n_resources
        ...
        resource_consumption[zoning, resource] =
            base * rand(rng, LogNormal(0.0, 0.16))
    end

    net_benefit = revenues .- development_costs
    reference_assignment = _land_use_reference_assignment(
        net_benefit, parcel_coordinates, neighbors, parity,
        min_counts_by_type,
    )
    ...
    for parcel in 1:n_parcels
        ...
        candidates = [zoning for zoning in 1:n_zoning_types
                      if zoning != reference_assignment[parcel]]
    ...
    reference_usage = [
        sum(parcel_sizes[parcel] *
            resource_consumption[reference_assignment[parcel], resource]
            for parcel in 1:n_parcels)
        for resource in 1:n_resources
    ]
    total_area = sum(parcel_sizes)
    average_consumption = vec(mean(resource_consumption; dims = 1))
    nominal_capacity = total_area .* average_consumption .* infrastructure_factor .*
    ...
    elseif feasibility_status == infeasible
        ...
        resource_capacities = reference_usage .*
                              rand(rng, Uniform(1.05, 1.20), n_resources)
        critical_resource = rand(rng, 1:n_resources)
        per_parcel_minimum = zeros(Float64, n_parcels)
        for parcel in 1:n_parcels
            allowed = [zoning for zoning in 1:n_zoning_types
                       if !environmental_restrictions[parcel, zoning]]
            minimum_rate = minimum(resource_consumption[zoning, critical_resource]
                                   for zoning in allowed)
        ...
        resource_capacities = nominal_capacity .*
                              rand(rng, Uniform(0.80, 1.20), n_resources)
    end
...
function build_model(prob::LandUseProblem)
    ...
    @objective(model, Max,
        sum(prob.parcel_sizes[parcel] *
            (prob.revenues[parcel, zoning] -
             prob.development_costs[parcel, zoning]) * x[parcel, zoning]
            for parcel in 1:prob.n_parcels, zoning in 1:prob.n_zoning_types))

    @constraint(model, parcel_assignment[parcel in 1:prob.n_parcels],
        sum(x[parcel, zoning] for zoning in 1:prob.n_zoning_types) == 1)

    @constraint(model, resource_capacity[resource in 1:prob.n_resources],
        sum(prob.parcel_sizes[parcel] *
            prob.resource_consumption[zoning, resource] * x[parcel, zoning]
            for parcel in 1:prob.n_parcels, zoning in 1:prob.n_zoning_types) <=
    ...
    if prob.minimum_zoning_requirements
        @constraint(model, minimum_zoning[zoning in eachindex(prob.min_counts_by_type)],
    ...
    if prob.zoning_adjacency_constraints && prob.n_zoning_types >= 3
        @constraint(model,
            residential_industrial_forward[
                edge in eachindex(prob.adjacency_edges)
            ],
            x[prob.adjacency_edges[edge][1], 1] +
            x[prob.adjacency_edges[edge][2], 3] <= 1)
        @constraint(model,
            residential_industrial_reverse[
                edge in eachindex(prob.adjacency_edges)
            ],
            x[prob.adjacency_edges[edge][1], 3] +
            x[prob.adjacency_edges[edge][2], 1] <= 1)
    end
