...
function _mss_profiles(spec_ids::Vector{Int}, n_days::Int)
    ...
    for s in 1:S
        p = _ORSCHED_SPECIALTIES[spec_ids[s]]
        cases_per_block = 480.0 / (p.aggregate_mean + 25.0)
        ward_prob = 1.0 - p.day_case
        mean_ward = max(0.5, (p.ward_los[1] + p.ward_los[2]) / 2)
        for lag in 0:(n_days - 1)
            icu[s, lag + 1] = cases_per_block * p.icu * (lag == 0 ? 1.0 : 0.35^(lag))
            direct = cases_per_block * (1 - p.icu) * ward_prob * exp(-lag / mean_ward)
            post_icu = lag == 0 ? 0.0 :
                       cases_per_block * p.icu * exp(-(lag - 1) / mean_ward)
            ward[s, lag + 1] = direct + post_icu
        end
    end
    return ward, icu
end

function OperatingRoomMasterScheduleProblem(target_variables::Int,
                                             feasibility_status::FeasibilityStatus,
                                             seed::Int)
    ...
    for r in randperm(rng, R), d in randperm(rng, D)
        ...
        choices = [s for s in 1:S if compatible[r, s] &&
                   sum(planted[s, :, d]) < max_daily[s]]
    ...
    function occupancy(profile)
        return [sum(profile[s, mod(d - dp, D) + 1] * planted[s, r, dp]
    ...
    admissible = [(s, r, d) for d in 1:D for r in 1:R for s in 1:S
                  if compatible[r, s]]
    ...
    return OperatingRoomMasterScheduleProblem(
        S, R, D, [_ORSCHED_SPECIALTIES[k].name for k in spec_ids],
        target_blocks, min_blocks, max_blocks, max_daily, compatible,
        admissible, preference, open_cost, ward_profile, icu_profile,
        ward_capacity, icu_capacity, under, over,
        rand(rng, Uniform(10.0, 30.0)), rand(rng, Uniform(20.0, 50.0)),
        witness, infeasible_specialty, feasibility_status,
    )
end
...
function build_model(prob::OperatingRoomMasterScheduleProblem)
    ...
    @constraint(model, room_exclusivity[r=1:R, d=1:D],
                sum(assign_block[a] for a in by_room_day[(r, d)]; init=0.0) ==
                open_room[r, d])
    for s in 1:S
        @constraint(model, sum(assign_block[a] for a in by_specialty[s]) +
                           under_blocks[s] - over_blocks[s] == prob.target_blocks[s])
    ...
    for d in 1:D
        ward_terms = [(a, prob.ward_profile[s, mod(d - dp, D) + 1])
                      for (a, (s, _, dp)) in enumerate(prob.admissible_blocks)]
        icu_terms = [(a, prob.icu_profile[s, mod(d - dp, D) + 1])
                     for (a, (s, _, dp)) in enumerate(prob.admissible_blocks)]
        @constraint(model, ward_occupancy[d] ==
                    sum(coef * assign_block[a] for (a, coef) in ward_terms))
        @constraint(model, icu_occupancy[d] ==
                    sum(coef * assign_block[a] for (a, coef) in icu_terms))
    ...
    @objective(model, Min,
        ...
        sum(prob.under_penalty[s] * under_blocks[s] +
            prob.over_penalty[s] * over_blocks[s] for s in 1:S) +
        prob.peak_ward_weight * peak_ward + prob.peak_icu_weight * peak_icu)
    return model
end
