...
function _mss_uniform_los_survival(los::Tuple{Int,Int}, age::Int;
                                   minimum_los::Int=0)
    age < 0 && return 0.0
    lo, hi = los
    return count(length_of_stay -> max(minimum_los, length_of_stay) > age,
                 lo:hi) / (hi - lo + 1)
end
...
function _mss_profile_components(profile, n_days::Int)
    ...
    for lag in 0:max_lag
        ...
        icu_survival = lag == 0 ? 1.0 : lag == 1 ? 0.5 : 0.0
        icu[phase] += cases_per_block * profile.icu * icu_survival

        direct_survival = _mss_uniform_los_survival(profile.ward_los, lag)
        direct_ward[phase] += cases_per_block * (1 - profile.icu) *
                              (1 - profile.day_case) * direct_survival

        if lag >= 1
            # Discharge after one ICU day (probability 1/2).
            post_icu_ward[phase] += cases_per_block * profile.icu * 0.5 *
                _mss_uniform_los_survival(profile.ward_los, lag - 1;
                                          minimum_los=1)
        end
        if lag >= 2
            # Discharge after two ICU days (probability 1/2).
            post_icu_ward[phase] += cases_per_block * profile.icu * 0.5 *
                _mss_uniform_los_survival(profile.ward_los, lag - 2;
                                          minimum_los=1)
        end
    end
