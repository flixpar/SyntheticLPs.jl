...
@testset "Nurse Scheduling" begin
    ...
    shortfalls = 0
    slots = 0
    for target in (50, 100, 500, 1000, 5000), status in (feasible, infeasible, unknown), seed in 0:5
        ...
        for d in 1:p.n_days, s in 1:p.n_shifts
            slots += 1
            sum(p.availability[:, d, s]) < p.min_available_per_shift && (shortfalls += 1)
        end
    ...
    for target in (50, 500, 5000), seed in 0:2
        ...
        @test all(
            A[n, d, s] <= p.availability[n, d, s] for
            n in 1:p.n_nurses, d in 1:p.n_days, s in 1:p.n_shifts
        )
        @test all(sum(A[n, d, :]) <= 1 for n in 1:p.n_nurses, d in 1:p.n_days)

        # Coverage and skill mix.
        @test all(sum(A[:, d, s]) >= p.demand[d, s] for d in 1:p.n_days, s in 1:p.n_shifts)
        @test all(
            sum(p.nurse_skills[n, k] * A[n, d, s] for n in 1:p.n_nurses) >=
            p.skill_requirements[d, s, k] for d in 1:p.n_days, s in 1:p.n_shifts, k in 2:n_skills
        )

        # Per-nurse aggregates match the roster and respect their bounds.
        @test w.shift_totals == [sum(A[n, :, :]) for n in 1:p.n_nurses]
        @test all(p.min_shifts[n] <= w.shift_totals[n] <= p.max_shifts[n] for n in 1:p.n_nurses)
        @test w.weekend_counts ==
            [sum(A[n, d, s] for d in p.weekend_days, s in 1:p.n_shifts) for n in 1:p.n_nurses]
        @test all(
            p.weekend_bounds[n][1] <= w.weekend_counts[n] <= p.weekend_bounds[n][2] for
            n in 1:p.n_nurses
        )
        @test w.night_counts == [sum(A[n, d, night_idx] for d in 1:p.n_days) for n in 1:p.n_nurses]
        ...
        @test all(w.max_consecutive[n] <= p.max_consecutive_days[n] for n in 1:p.n_nurses)
        @test all(
            A[n, d, night_idx] + A[n, d + offset, idx] <= 1 for n in 1:p.n_nurses for
            d in 1:p.n_days for offset in 1:p.rest_after_night[n] if d + offset <= p.n_days for
            idx in early
        )
    end
    ...
    statuses = [
        generate_problem(:nurse_scheduling, 200, unknown, s)[2].feasibility_status for s in 0:19
    ]
