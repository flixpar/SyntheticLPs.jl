...
@testset "Process Planning" begin
    ...
    @test Set(list_variants(:process_planning)) ==
          Set([:refinery, :mode_switching, :hydrogen_network, :campaign,
               :capacity_expansion])
    ...
    refinery_variables(p) = p.data.n_periods * (
        ...
        length(p.flowsheet.storable) + length(p.flowsheet.purchasable) +
        length(p.flowsheet.spot) + 2 * length(p.flowsheet.products))

    # The same, with feeds replicated per operating mode and each mode carrying a
    # throughput, a run indicator and a changeover.
    mode_variables(p) = p.data.n_periods * (
        3 * p.flowsheet.n_crudes +
        sum(length(u.modes) * (length(u.feeds) + 3) for u in p.flowsheet.units;
            init=0) +
        sum(length(g.components) for g in p.flowsheet.products; init=0) +
        length(p.flowsheet.storable) + length(p.flowsheet.purchasable) +
        length(p.flowsheet.spot) + 2 * length(p.flowsheet.products))

    expansion_variables(p) = p.n_periods * (
        4 * length(p.technologies) + length(p.raw_chemicals) +
        length(p.sellable_chemicals))

    hydrogen_variables(p) = refinery_variables(p) + 6 * p.data.n_periods

    campaign_tasks(p) =
        [t for t in eachindex(p.task_names) if p.campaign_unit[p.task_unit[t]]]

    campaign_variables(p) = p.n_periods * (
        length(p.task_names) + 2 * length(campaign_tasks(p)) +
        count(==(:raw), p.material_kind) * p.n_tiers +
        length(p.material_names) + count(==(:final), p.material_kind))

    campaign_rows(p) = p.n_periods * (
        length(p.material_names) + length(p.unit_names) +
        count(p.campaign_unit) + 6 * length(campaign_tasks(p)))
    ...
    @testset "Variable counts and sizing" begin
        for target in (60, 240, 1500, 9000), status in (feasible, infeasible, unknown)
            ...
            m, p = generate_problem(:process_planning, target, status, 3;
                                    variant=:mode_switching)
            @test num_variables(m) == mode_variables(p)
            @test p.feasibility_status == status

            m, p = generate_problem(:process_planning, target, status, 3;
                                    variant=:hydrogen_network)
    ...
    @testset "Flowsheet invariants" begin
        for variant in (:refinery, :mode_switching, :hydrogen_network),
            target in (120, 900, 6000), status in (feasible, infeasible, unknown)
            ...
            @test all(isapprox(sum(fs.cut_yields[c, :]), 1.0; atol=1e-9)
            ...
            for grade in fs.products
                ...
                @test all(grade.spec_min[q] <= grade.spec_max[q]
            ...
            @test all(p.data.stream_initial_inventory .<=
                      p.data.stream_tank_capacity .+ 1e-9)
            @test all(p.data.crude_initial_inventory .<=
                      p.data.crude_tank_capacity .+ 1e-9)
            @test 0 <= p.data.renewable_min_fraction <=
                  p.data.renewable_max_fraction <= 1
            ...
            # a merchant outlet.
            upgraded = Set((:reformate_mid, :reformate_high, :alkylate,
                            :isomerate, :ulsd_component, :hc_diesel,
                            :jet_component))
    ...
    @testset "Planted operation" begin
        for variant in (:refinery, :mode_switching)
            for target in (150, 800, 4000), seed in 0:2
                ...
                @test all(1 <= plan.unit_mode[u, t] <= length(fs.units[u].modes)
                          for u in eachindex(fs.units), t in 1:T)
                ...
                for (g, grade) in enumerate(fs.products), t in 1:T
                    ...
                    for q in 1:PP.PP_N_QUALITIES
                        ...
                        isfinite(grade.spec_min[q]) &&
                            @test quality[q] >= grade.spec_min[q] - tol
                        isfinite(grade.spec_max[q]) &&
                            @test quality[q] <= grade.spec_max[q] + tol
                    end
                end
            end
            ...
            for status in (infeasible, unknown)
                _, p = generate_problem(:process_planning, 400, status, 1;
                                        variant=variant)
        ...
        _, p = generate_problem(:process_planning, 4000, feasible, 4;
                                variant=:mode_switching)
        multimode = [u for u in eachindex(p.flowsheet.units)
                     if length(p.flowsheet.units[u].modes) > 1]
        if !isempty(multimode) && p.data.n_periods >= 4
            @test any(length(unique(p.feasible_witness.unit_mode[u, :])) > 1
                      for u in multimode)
        end
        ...
        for target in (50, 500, 3000), seed in 0:4
            ...
            demand = sum(p.hydrogen.unit_h2_rate[u][f] *
                         w.refinery.unit_feed[u][f, t]
                         for u in eachindex(p.flowsheet.units)
                         for f in eachindex(p.flowsheet.units[u].feeds)
                         for t in 1:p.data.n_periods)
            @test demand > 0
            @test sum(w.sulfur_recovered) > 0
            @test sum(w.carbon_emissions) > 0
        end
        for status in (infeasible, unknown)
            _, p = generate_problem(:process_planning, 500, status, 2;
                                    variant=:hydrogen_network)
            @test p.feasible_witness === nothing
        end
    end
    ...
    @testset "Infeasibility certificates" begin
        for variant in (:refinery, :mode_switching, :hydrogen_network)
            ...
            for target in (150, 800, 4000), seed in 0:5
                _, p = generate_problem(:process_planning, target, infeasible, seed;
                                        variant=variant)
                ...
                if certificate.kind == PP.refinery_contract_above_conversion_bound
                    ...
                    @test isapprox(certificate.achievable,
                                   PP._pp_production_bound(p.flowsheet, p.data);
                                   rtol=1e-9)
                    @test isapprox(certificate.required, sum(p.data.demand_min);
                                   rtol=1e-9)
                else
                    ...
                    values = [p.flowsheet.qualities[s, certificate.quality]
                              for s in grade.components]
            ...
            _, feasible_problem = generate_problem(:process_planning, 400, feasible, 2;
                                                   variant=variant)
            _, broken = generate_problem(:process_planning, 400, infeasible, 2;
                                         variant=variant)
            @test !PP.refinery_certificate_holds(feasible_problem.flowsheet,
                                                 feasible_problem.data,
                                                 broken.infeasibility_certificate)
        end
    end

    @testset "Process campaigns" begin
        for target in (60, 600, 5000, 20000), status in
            ...
            @test all((count(==(u), p.task_unit) > 1) == p.campaign_unit[u]
                      for u in eachindex(p.unit_names))
            @test all(c > 0 for io in (p.task_inputs..., p.task_outputs...)
                      for (_, c) in io)
        ...
        @test_throws ArgumentError generate_problem(
            :process_planning, PP.MAX_CAMPAIGN_PLANNING_VARIABLES + 1,
    ...
    @testset "Capacity expansion network" begin
        for target in (120, 900, 6000), status in (feasible, infeasible, unknown)
            ...
            @test all(!(p.chemicals[j].purchasable && p.chemicals[j].sellable)
                      for j in 1:J)
            for technology in p.technologies
                # Inputs come from strictly lower layers, so the network is a DAG.
                @test all(p.chemicals[j].layer < technology.layer
                          for (j, _) in technology.inputs)
        ...
        for target in (150, 900, 5000), seed in 0:5
            ...
            @test p.infeasibility_certificate.required >
                  p.infeasibility_certificate.achievable
        end
        @test length(kinds) == 2

        # A feasible instance's data does not support the certificate of a
        # different, broken one.
        _, healthy = generate_problem(:process_planning, 600, feasible, 3;
                                      variant=:capacity_expansion)
        _, broken = generate_problem(:process_planning, 600, infeasible, 3;
                                     variant=:capacity_expansion)
        transplanted = PP.ProcessCapacityExpansionProblem(
            healthy.n_periods, healthy.chemicals, healthy.technologies,
            healthy.raw_chemicals, healthy.sellable_chemicals, healthy.purchase_cost,
            healthy.availability, healthy.sale_price, healthy.demand_min,
            healthy.demand_max, healthy.discount, nothing,
            broken.infeasibility_certificate, infeasible)
    ...
    @testset "Model structure" begin
        ...
        # mode, and period.
        m, p = generate_problem(:process_planning, 2000, unknown, 2;
                                variant=:mode_switching, relax_integer=false)
        @test num_constraints(m, VariableRef, MOI.ZeroOne) ==
              2 * p.data.n_periods *
              sum(length(u.modes) for u in p.flowsheet.units; init=0)
        @test num_variables(m) == mode_variables(p)
        relaxed, _ = generate_problem(:process_planning, 2000, unknown, 2;
                                      variant=:mode_switching)
        ...
        m, p = generate_problem(:process_planning, 2000, unknown, 2;
                                variant=:capacity_expansion, relax_integer=false)
        @test num_constraints(m, VariableRef, MOI.ZeroOne) ==
              p.n_periods * length(p.technologies)

        # Reproducibility: identical data and model for the same seed.
        for variant in (:refinery, :mode_switching, :hydrogen_network,
                        :campaign, :capacity_expansion)
            m1, p1 = generate_problem(:process_planning, 700, feasible, 11;
                                      variant=variant)
            m2, p2 = generate_problem(:process_planning, 700, feasible, 11;
                                      variant=variant)
        ...
        for target in (1, 5, 25), variant in
            (:refinery, :mode_switching, :hydrogen_network, :campaign,
             :capacity_expansion),
            status in (feasible, infeasible, unknown)
            m, _ = generate_problem(:process_planning, target, status, 0;
                                    variant=variant)
        ...
        if HAS_HIGHS
            ...
            for variant in (:refinery, :mode_switching, :hydrogen_network,
                ...
                for target in (120, 900, 4000), seed in 0:2
                    m, _ = generate_problem(:process_planning, target, feasible, seed;
                                            variant=variant)
            ...
            for variant in (:mode_switching, :campaign, :capacity_expansion)
                m, _ = generate_problem(:process_planning, 600, feasible, 1;
                                        variant=variant, relax_integer=false)
