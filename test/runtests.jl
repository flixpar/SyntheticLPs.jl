...
@testset "SyntheticLPs" begin
    ...
    @testset "Operating Room Scheduling" begin
        variants = [:benchmark_loading, :case_sequencing, :elective_assignment,
                    :master_surgical_schedule, :robust_elective, :weekly_planning]
        ...
        for (rooms, days, specs) in ((2, 5, 3), (5, 10, 7), (8, 10, 11)), seed in 1:100
            ...
            @test all((mss[r, d] == 0) == (session[r, d] == 0)
                      for r in 1:rooms, d in 1:days)
        end
        ...
        for seed in 0:3, target in (100, 500)
            ...
            @test all(count(==(k), p.mss) >= max(1, p.n_days ÷ 5)
        ...
        for seed in 0:3, target in (100, 500)
            ...
            @test num_variables(model) == sum(length, p.eligible_rooms) +
                  sum(length, p.eligible_surgeons) + length(p.room_pairs) +
                  length(p.surgeon_pairs) + 2p.n_surgeries + 1
            expected_rooms = Tuple{Int,Int,Int}[]
            expected_surgeons = Tuple{Int,Int,Int}[]
            for o in 1:p.n_surgeries, q in (o + 1):p.n_surgeries
                append!(expected_rooms, [(o, q, r) for r in intersect(p.eligible_rooms[o], p.eligible_rooms[q])])
                append!(expected_surgeons, [(o, q, s) for s in intersect(p.eligible_surgeons[o], p.eligible_surgeons[q])])
            end
            ...
            for o in 1:p.n_surgeries
                ...
                @test all(finish + p.room_turnover <= a + 1e-9 ||
                          b + p.room_turnover <= t + 1e-9
                          for (a, b) in get(room_seen, r, []))
                @test all(finish + p.surgeon_turnover <= a + 1e-9 ||
        ...
        for seed in 0:3, target in (100, 500)
            ...
            @test num_variables(model) == length(p.admissible_blocks) +
                  p.n_rooms * p.n_days + 2p.n_specialties + 2p.n_days + 2
            @test Set(p.admissible_blocks) == Set((s, r, d) for d in 1:p.n_days,
                r in 1:p.n_rooms, s in 1:p.n_specialties if p.room_specialty_compatible[r, s])
            w = something(p.feasible_witness)
            ...
            for s in 1:p.n_specialties
                ...
                @test all(w[s, r, d] == 0 || p.room_specialty_compatible[r, s]
                          for r in 1:p.n_rooms, d in 1:p.n_days)
            end
            for d in 1:p.n_days
                ward = sum(p.ward_profile[s, mod(d - dp, p.n_days) + 1] * w[s, r, dp]
                           for s in 1:p.n_specialties, r in 1:p.n_rooms, dp in 1:p.n_days)
                icu = sum(p.icu_profile[s, mod(d - dp, p.n_days) + 1] * w[s, r, dp]
                          for s in 1:p.n_specialties, r in 1:p.n_rooms, dp in 1:p.n_days)
        ...
        for seed in 0:3, target in (100, 500)
            ...
            @test num_variables(model) == 2length(p.admissible) + p.n_surgeries +
                  2length(p.open_blocks)
            ...
            for (q, (r, d)) in enumerate(p.open_blocks)
                ...
                robust = SyntheticLPs._robust_extra_capacity(p.duration_deviation[cases],
                                                              p.uncertainty_budget[q])
        ...
        for seed in 0:3, target in (50, 200, 500)
            ...
            @test num_variables(model) == p.n_surgeries * p.n_or_days +
                  p.n_surgeries + p.n_or_days
            @test p.target_load in collect(0.80:0.05:1.20)
            @test abs(p.achieved_load - p.target_load) <= 0.025 + 1e-9
            @test all(isapprox(p.expected_duration[i], p.duration_gamma[i] +
                               exp(p.duration_mu[i] + p.duration_sigma[i]^2 / 2))
                      for i in 1:p.n_surgeries)
    ...
    @testset "Hub Location" begin
        ...
        path_count(A) =
            sum(length(A[i]) * length(A[j])
                for i in 1:length(A) for j in (i + 1):length(A)) +
            sum(length(a) for a in A) + length(union(A...))
        ...
        for status in (feasible, infeasible)
            ...
            @test num_variables(m) ==
                  sum(sum(length(A[i]) for i in 1:n if i != j) + h * (h - 1) +
        ...
        for v in list_variants(:hub_location), target in (50, 200, 1000, 4000),
            status in (feasible, infeasible, unknown), seed in 0:3
            m, _ = generate_problem(ProblemVariant(:hub_location, v), target,
                                    status, seed)
            nv = num_variables(m)
        ...
        for v in (:p_hub_median, :r_allocation)
            ...
            @test all(all(p.dist[i, k] <= p.reach + 1e-9 for k in p.admissible[i])
        ...
        for v in (:p_hub_median, :r_allocation), s in 0:2
            _, p = generate_problem(ProblemVariant(:hub_location, v), 150,
                                    infeasible, s)
            cert = p.infeasibility_certificate
            @test cert !== nothing
            ...
            for a in 1:length(cert.groups), b in (a + 1):length(cert.groups)
                @test isempty(intersect(group_admissibles(p, cert.groups[a]),
                                        group_admissibles(p, cert.groups[b])))
            end
        ...
        @test all(any(k in p.admissible[i] for k in w.open_hubs)
                  for i in 1:p.n_nodes)
        @test p.budget >= sum(p.fixed_cost[position[k]] for k in w.open_hubs)
        for s in 0:2
            _, q = generate_problem("hub_location/multiple_allocation", 150,
                                    infeasible, s)
            cert = q.infeasibility_certificate
            @test cert !== nothing
            @test q.feasible_witness === nothing
            @test cert.budget < cert.minimum_fixed_cost
            @test cert.minimum_fixed_cost ==
                  length(cert.groups) * minimum(q.fixed_cost)
            # Disjoint admissible sets across groups.
            for a in 1:length(cert.groups), b in (a + 1):length(cert.groups)
                @test isempty(intersect(
                    group_admissibles(q, cert.groups[a]),
                    group_admissibles(q, cert.groups[b])))
            end
        end
        ...
        for s in 0:2
            ...
            cert = q.infeasibility_certificate
            @test cert !== nothing
            recomputed = sum(q.flow[i, j] + q.flow[j, i]
                             for i in cert.side_a, j in 1:q.n_nodes
                             if !(j in cert.side_a))
            @test cert.crossing_flow ≈ recomputed
            crossing = [t for (t, (k, m)) in enumerate(q.links)
                        if ((k in cert.side_a) != (m in cert.side_a))]
            @test cert.crossing_capacity ==
                  sum(q.link_capacity[t] for t in crossing)
        ...
        if HAS_HIGHS
            ...
            for v in list_variants(:hub_location), s in 0:9
                m, _ = generate_problem(ProblemVariant(:hub_location, v), 200,
                                        unknown, s)
                set_optimizer(m, HiGHS.Optimizer)
                set_silent(m)
                optimize!(m)
