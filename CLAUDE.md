...
- `dualize_model(model)` / `dual_reformulation(model)`: returns a separately named dual model while leaving the primal unchanged. Dualization is off by default. `generate_random_problem` and `generate_dataset` accept `dualize_probability=0.0` for reproducible per-instance primal/dual sampling; `dualize=true` remains the force-all override. The CLI equivalents are `--dualize-probability` and `--dualize`. The transform rejects unrelaxed discrete variables, splits ranged affine rows on an internal copy, and runs after relaxation and bounds reformulation. Feasibility-contract verification applies to the source primal before dualization; dataset size and quality metadata apply to the returned model, and `GeneratedInstance.dualized` records the choice.
- The package requires Julia 1.11+ and Dualization 0.7+.
...
The system includes 45 categories covering major LP/MIP problem classes. Each
...
`radiotherapy` (`:weighted_deviation`), `vehicle_routing` (`:cvrp`), and
...
- TSP (`standard`, `asymmetric`, `flow`, `time_windows`, `assignment_relaxation`, `prize_collecting`, `multiple_salespersons`, `precedence`), Vehicle Routing (`cvrp`), Regression (`lad`, `quantile`, `chebyshev`, `basis_pursuit`), Radiotherapy (`weighted_deviation`, `mean_tail_dose`, `minmax_deviation`, `robust_fluence`, `beam_angle_selection`), Inverse Optimization (`standard`, `classical_normalized`, `linf`, `noisy_observations`, `restricted_optimal_value`, `shortest_path`, `shortest_path_layered`, `market_clearing`), Workforce Shift Scheduling (`covering`), Bin Packing (`standard`, `heterogeneous`), Revenue Management (`standard`, `stochastic_overbooking`)
...
- **Pure LPs**: continuous formulations (e.g. transportation variants, diet
  ...
  Four `radiotherapy` variants are also pure LPs: summed or minimax voxelwise
  fluence-map optimization, convex mean-tail-dose planning, and deterministic-
  equivalent setup-robust planning, each with beamlet total variation.
- **MIPs** (binary/integer variables): e.g. `facility_location` variants
  ...
  `radiotherapy/beam_angle_selection` (binary candidate-field openings),
